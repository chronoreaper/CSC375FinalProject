...
def compute_traj_coeffs(initial_state, final_state, tf):
    ...
    output: coeffs (np.array shape [8]), coefficients on the basis functions
    ...
    A = np.array([[1, 0, 0, 0, 0, 0, 0, 0],  #x_0
                 [1, tf, tf**2, tf**3, 0, 0, 0, 0], #x_f
                 [0, 1, 0, 0, 0, 0, 0, 0], #xd_0
                 [0, 1, 2*tf, 3*tf**2, 0, 0, 0, 0], #xd_f
                 [0, 0, 0, 0, 1, 0, 0, 0], #y_0
                 [0, 0, 0, 0, 1, tf, tf**2, tf**3], #y_f
                 [0, 0, 0, 0, 0, 1, 0, 0], #yd
                 [0, 0, 0, 0, 0, 1, 2*tf, 3*tf**2]#yd_f
                 ])
    B = np.array([z_0.x, z_f.x, z_0.xd, z_f.xd, z_0.y, z_f.y, z_0.yd, z_f.yd])
...
def compute_traj(coeffs, tf, N):
    """
    inputs: coeffs (np.array shape [8]), coefficients on the basis functions
            tf (float) final_time
            N (int) number of points
    output: traj (np.array shape [N,6]), N points along the trajectory, from t=0
    ...
    traj[:,3] = coeffs[1] + coeffs[2]*2*t + coeffs[3]*3*t**2
    traj[:,4] = coeffs[5] + coeffs[6]*2*t + coeffs[7]*3*t**2
    traj[:,5] = coeffs[2]*2 + coeffs[3]*6*t
    traj[:,6] = coeffs[6]*2 + coeffs[7]*6*t
...
def compute_controls(traj):
    """
    inputs: traj (np.array shape [N,6])
    outputs: V (np.array shape [N]) V at each point of traj
             om (np.array shape [N]) om at each point of traj
    """

    ### begin code
    V = linalg.norm(traj[:,3:5], axis=-1)
    om = (traj[:,6]*traj[:,3] - traj[:,5]*traj[:,4])*1./(V**2)
    ### end code
...
def rescale_V(V, om, V_max, om_max):
    ...
    '''
    ########## Code starts here ##########
    V_tilde = np.minimum(np.minimum(abs(V), V_max), om_max*abs(V*1./om))*np.sign(V)
    ########## Code ends here ##########
    return V_tilde
...
def interpolate_traj(traj, tau, V_tilde, om_tilde, dt, s_f):
    '''
    inputs: traj (np.array [N,6]) original unscaled trajectory
    ...
    outputs: t_new (np.array [N_new]) new timepoints spaced dt apart
             V_scaled (np.array [N_new])
             om_scaled (np.array [N_new])
             traj_scaled (np.array [N_new, 6]) new rescaled traj at these timepoints
    '''
    # Get new final time
    tf_new = tau[-1]
